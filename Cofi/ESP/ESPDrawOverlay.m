...
static void esp_draw_overlay_ensure_player_slot(NSUInteger index, UIView *parent) {
    ...
    slot->boxLayer.strokeColor = [UIColor whiteColor].CGColor;
    slot->boxLayer.lineWidth = 0.5f;
...
int esp_draw_overlay_initialize_in_session(void) {
    ...
    void (^setupBlock)(void) = ^{
        if (!gOverlayWindow) {
            ...
            gCountLabel.font = [UIFont boldSystemFontOfSize:24.0f];
            gCountLabel.textColor = [UIColor colorWithRed:1.0 green:0.15 blue:0.15 alpha:1.0];
...
static void esp_draw_overlay_apply_box(ESPPlayerSlotView *slot, CGRect box, BOOL enabled) {
    ...
    slot->boxLayer.lineWidth = 0.5f;
    slot->boxLayer.path = esp_corner_box_path(box).CGPath;
