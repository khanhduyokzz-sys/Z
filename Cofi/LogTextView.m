...
void log_session_begin(void) {
    ...
    if (log_file) {
        ...
        fprintf(log_file,
                "# Cofi chain session %04d-%02d-%02d %02d:%02d:%02d\n",
...
static UIColor *baseColorForLogLine(NSString *line) {
...
// White log surface: the tag palette above was tuned for the old dark theme,
// so any still-bright color is pulled down toward readable darkness. Every
// tag keeps its hue but stays legible on the pure-white background.
static UIColor *colorForLogLine(NSString *line) {
    UIColor *base = baseColorForLogLine(line);
    CGFloat r = 0, g = 0, b = 0, a = 0;
    if ([base getRed:&r green:&g blue:&b alpha:&a]) {
        CGFloat luminance = 0.299 * r + 0.587 * g + 0.114 * b;
        if (luminance > 0.62) {
            CGFloat k = 0.62 / luminance;
...
- (void)setup {
    self.editable   = NO;
    self.font = [[UIFontMetrics metricsForTextStyle:UIFontTextStyleCaption1]
        scaledFontForFont:[UIFont monospacedSystemFontOfSize:12 weight:UIFontWeightRegular]];
    self.adjustsFontForContentSizeCategory = YES;
    self.backgroundColor = UIColor.whiteColor;
    self.textColor  = [UIColor colorWithWhite:0.18 alpha:1.0];
...
- (NSMutableAttributedString *)buildAttrStringForText:(NSString *)text {
    ...
    UIFont *boldFont = [UIFont monospacedSystemFontOfSize:self.font.pointSize weight:UIFontWeightSemibold];
