...
// Classic-white palette. Calm teal accent + graphite text + 2% graphite
// muted surface. Every state colour (live/warn/danger) is desaturated so the
// dashboard reads as a single calm iOS surface instead of a tech demo.
static inline UIColor *DSAccentTeal(void) {
    return [UIColor colorWithRed:0.059f green:0.463f blue:0.431f alpha:1.0f]; // #0F766E
}

static inline UIColor *DSAccentSoft(void) {
    return [UIColor colorWithRed:0.078f green:0.722f blue:0.651f alpha:1.0f]; // #14B8A6
}

static inline UIColor *DSLiveColor(void) {
    return [UIColor colorWithRed:0.086f green:0.639f blue:0.290f alpha:1.0f]; // #16A34A emerald
}

static inline UIColor *DSWarnColor(void) {
    return [UIColor colorWithRed:0.851f green:0.467f blue:0.024f alpha:1.0f]; // #D97706 amber
}

static inline UIColor *DSTealColor(void) {
    return [UIColor colorWithRed:0.059f green:0.463f blue:0.431f alpha:1.0f]; // teal (alias)
}

static inline UIColor *DSGraphiteColor(void) {
    return [UIColor colorWithWhite:0.10f alpha:1.0f]; // near-black graphite text
}
...
- (void)viewDidLoad {
    ...
    // Force Light trait so the Game dashboard stays classic-white regardless
    // of the iOS system appearance.
...
- (UITableViewCell *)heroCell {
    ...
    UILabel *brand = DSCapsLabel(@"DSWUNITY  ·  CLASSIC WHITE", 10, UIFontWeightSemibold,
    ...
    UIColor *heroGradientEnd = _currentStep == ESPStepFailed
        ? [UIColor colorWithRed:0.863f green:0.149f blue:0.149f alpha:1.0f] // danger red
        : DSAccentSoft();
...
- (UITableViewCell *)actionCell {
    ...
    fill.gradientColors = _espActive
        ? @[[UIColor colorWithRed:0.863f green:0.149f blue:0.149f alpha:1.0f],  // danger
            [UIColor colorWithRed:0.851f green:0.467f blue:0.024f alpha:1.0f]]  // warn amber
        : @[DSAccentTeal(), DSAccentSoft()];
...
- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section {
    return section == ESPSectionSupport ? @"DSWUnity · Classic White Overlay Engine · t.me/duydzne" : nil;
}
