...
static void hexdump_file(const char *path, size_t max_bytes) {
...
int grab_kernelcache(void)
{
    ...
    if(kc_folder_vnode == 0 || kc_folder_vnode == (uint64_t)-1) {
        printf("Failed to get kernelcache folder vnode: %s\n", kc_folder_path.UTF8String);
        return -1;
    }
