...
void settings_register_defaults(void) {
    [[NSUserDefaults standardUserDefaults] registerDefaults:@{
        ...
        kSettingsAimSpeed:          @1000.0,  // Kept for ABI compat; aim engine hardcodes VIP constants
        kSettingsAimPos:            @0,
        ...
        // Preset the theme to white (0 = light). The classic-white theme is
        // the only shipped appearance; this default ensures first launch and
        // every reset lands on white without depending on the iOS system
        // appearance.
        UIThemePreferenceKey:       @0,
    }];
}
...
- (void)viewDidLoad {
    ...
    // Force Light trait so every pushed Settings screen stays classic-white
    // regardless of the iOS system appearance.
...
- (void)buildAboutRows {
    ...
    _sectionFooters = @[@"DSWUnity · Classic white overlay for Free Fire"];
}

#pragma mark - Root Rows

- (void)buildRootRows {
    // Classic white navigation: every entry uses the calm graphite + teal
    // accent system. The three top-level destinations mirror the user-facing
    // mental model of the app: System → Overlay → Aim → About.
    UIColor *teal  = [UIColor colorWithRed:0.059f green:0.463f blue:0.431f alpha:1.0f];
    UIColor *indigo = UIColor.systemIndigoColor;
    UIColor *graphite = [UIColor colorWithRed:0.35f green:0.45f blue:0.58f alpha:1.0f];

    NSArray *quickActions = @[
        SettingsRow(@"paperplane.fill", teal, @"Telegram Contact",
    ...
    NSArray *menu = @[
        SettingsRow(@"square.3.layers.3d.down.right", teal, @"ESP Configuration",
                    @"Display, line, box, health, name, distance, counter & refresh rate.", nil, SettingsRowKindDisclosure, nil),
    ];

    NSArray *more = @[
        SettingsRow(@"paintpalette", indigo, @"Appearance",
                    @"Theme and background modes.", nil, SettingsRowKindDisclosure, nil),
        SettingsRow(@"info.circle", graphite, @"About DSWUnity",
    ...
    _sectionFooters = @[@"", @"", @"DSWUnity · Classic White Overlay Engine"];
}
...
- (void)buildESPRows {
    ...
    // Section 1: ESP Display Toggles — the per-feature visibility switches,
    // grouped in the order the user mentally scans the screen (from line
    // at the top, down to the count indicator at the bottom).
    NSArray *drawSection = @[
    ...
    // Section 2: Performance — the only knob here today is the roster tick
    // rate, kept on its own row so users can change it without scrolling
    // past the aim module.
    NSArray *espSection = @[
        StepperRow(@"ESP Refresh Rate (Hz)", @"1–30 Hz, default 30", kSettingsESPRateTick, 1, 30),
    ];

    // Section 3: Aim Engine (VIP) — full VIP-standard aim module. Order
    // matches the VIP menu: enable → FOV → lock line → radius → position →
    // trigger → filters. This is the canonical aim config.
    //
    // Note: the AimSpeed slider is intentionally removed — VIP's reference
    // Slerp constants (0.97/0.92/0.85) are hardcoded inside AimAssistEngine
    // so the aim feels exactly like VIP. Exposing a slider would let users
    // deviate from the VIP feel, which defeats the 1:1 parity goal.
    NSArray *aimSection = @[
        ToggleRow(@"Aimbot", kSettingsAimbot),
        ToggleRow(@"Draw FOV Circle", kSettingsShowFov),
        ToggleRow(@"Draw Aim Lock Line", kSettingsAimLine),
        SliderRow(@"FOV Radius", @"Aim circle size in pixels", kSettingsAimFov, 5.0f, 500.0f, @"px"),
    ...
    _sectionFooters = @[
        ...
        @"Refresh rate per second (1 – 30 Hz). Higher rates feel smoother but consume more battery.",
        @"Kernel-steered aim assist, 1:1 with VIP. Slerp constants hardcoded (0.97 firing / 0.92 scoping / 0.85 normal). FOV circle draws at screen centre; selection window is 3× the circle radius. Trigger F+S also auto-fires while firing."
    ];
}
...
- (void)updateESPMenuButton {
    ...
    UIColor *tint = running
        ? [UIColor colorWithRed:0.059f green:0.463f blue:0.431f alpha:1.0f]
        : UIColor.labelColor;
