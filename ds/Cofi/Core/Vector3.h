...
float Vector3::Angle(Vector3 a, Vector3 b) {
    ...
    if (magProduct < 1e-6f || !isfinite(magProduct)) return 0.0f;
    float v = Dot(a, b) / magProduct;
...
Vector3 Vector3::Normalized(Vector3 v) {
    ...
    // !(mag > 0) also catches NaN magnitudes from non-finite input.
    if (!(mag > 0.0f))
        return Vector3::zero();
    return v / mag;
}
...
Vector3 Vector3::SlerpUnclamped(Vector3 a, Vector3 b, float t)
{
    ...
    if (!(magA > 1e-6f) || !isfinite(magA) || !(magB > 1e-6f) || !isfinite(magB)) {
        return Lerp(a, b, t);
    }
    a /= magA;
    b /= magB;
