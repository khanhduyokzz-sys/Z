...
//
// Faithful 1:1 port of the reference Vip build's aimbot pipeline:
...
//
// 1:1 parity with VIP:
//   * Slerp constants are HARDCODED (0.97 firing / 0.92 scoping / 0.85
//     normal) so users can't deviate from the VIP reference feel.
//   * PredictTargetPosition reads the TARGET pawn's weapon data internally
//     (matching VIP's behaviour, even though it's technically a bug — the
//     local player's weapon would be correct for bullet travel time).
//   * Auto-fire always writes the rotation after the fire-state set,
//     regardless of whether the fire-state write succeeded.
//   * All writes flow through RemoteWriteDomain::Aim and fail closed when
//     Aimbot is off.
...
// Reference PredictTargetPosition — 1:1 with VIP's esp.mm.
//
// VIP's pipeline reads the TARGET pawn's weapon data inside the prediction
// (not the local player's). This is technically a bug — you want YOUR bullet's
// travel time, not the enemy's — but it produces the reference VIP "feel", so
// we replicate it exactly for 1:1 parity.
//
// On the first encounter with a pawn (tracker cold), VIP returns currentPos
// unchanged (no prediction). We do the same.
static Vector3 PredictTargetPosition(uint64_t pawn,
    ...
    if (idx == -1) {
        if (gVelocityTrackerCount >= kVelocityTrackerCapacity) return currentPos;
        idx = gVelocityTrackerCount++;
        gVelocityTracker[idx].pawn = pawn;
        gVelocityTracker[idx].lastPos = currentPos;
        gVelocityTracker[idx].velocity = gameVel;
        gVelocityTracker[idx].lastTime = now;
        // Match VIP: no prediction on first encounter — return currentPos
        // unchanged so the aim stays on the raw head position this frame.
        return currentPos;
    }
    ...
    // 3. Weapon data — VIP reads the TARGET pawn's weapon inside the
    // prediction. This is the deliberate 1:1 replication of VIP behavior.
    AimWeaponData weapon = GetWeaponData(pawn);
...
#pragma mark - Aim steering (reference set_aim — 1:1 with VIP)
...
//
// Slerp constants are HARDCODED to match VIP exactly (0.97 firing / 0.92
// scoping / 0.85 normal). The AimSpeed preference is intentionally a no-op
// so users can't accidentally make aim slower or faster than the VIP
// reference feel.
static bool AimWriteSteering(uint64_t myPawn,
    ...
    // VIP reference Slerp factors — hardcoded for 1:1 parity.
    float t = isFiring ? 0.97f : (isScoping ? 0.92f : 0.85f);
...
- (AimAssistOutcome)processSnapshot:(ESPFrameSnapshot *)snapshot
    ...
    if (bestTarget && shouldAim && bestDistance >= 0.2f) {
        // 1:1 with VIP: PredictTargetPosition reads the target's weapon data
        // internally. The local player's weapon is only consulted for the
        // single-shot snap branch below.
        ...
        if (!AimIsZeroVec(finalTarget)) {
            ...
            // 1:1 with VIP: always write the rotation after the fire-state
            // set, regardless of whether the fire-state write succeeded.
