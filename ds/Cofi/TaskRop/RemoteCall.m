...
#define CY_VPHONE_BRIDGE_SOCK "/private/var/mobile/Library/Caches/Ds.Freefire.Esp.vphone-springboard.sock"
...
static void release_shmem_slot(int i)
{
    ...
    uint64_t mappingAddress = g_RC_shmemCache[i].mappingAddress
        ? g_RC_shmemCache[i].mappingAddress : g_RC_shmemCache[i].localAddress;
    uint64_t mappingSize = g_RC_shmemCache[i].mappingSize
        ? g_RC_shmemCache[i].mappingSize : PAGE_SIZE;
    if (mappingAddress) {
        mach_vm_deallocate(mach_task_self_,
                           (mach_vm_address_t)mappingAddress,
                           (mach_vm_size_t)mappingSize);
    }
...
struct VMShmem *get_shmem_for_page(uint64_t pageAddr)
{
    ...
    if (!newShmem.localAddress) {
        static volatile uint64_t shmemFailureEvents = 0;
        uint64_t events = __sync_add_and_fetch(&shmemFailureEvents, 1);
        if (events == 1 || (events % 64) == 0) {
            printf("[RemoteCall] shmem map failed page=0x%llx event=%llu\n",
...
bool remote_read_internal(uint64_t src, void *dst, uint64_t size)
{
    ...
    while (src < until) {
        ...
        if (!page) {
            static volatile uint64_t readFailureEvents = 0;
            uint64_t event = __sync_add_and_fetch(&readFailureEvents, 1);
            if (event == 1 || (event % 128ULL) == 0) {
                printf("[%s:%d] remote_read failed: unable to find remote page (event=%llu)\n",
                       __FUNCTION__, __LINE__, (unsigned long long)event);
            }
            return false;
        }
