#!/usr/bin/env bash
# Pull Cofi diagnostic logs from R2 into logs/ in this repo.
...
curl -fsS -H "X-Admin-Token: $ADMIN_TOKEN" \
  "$WORKER_URL/admin/list?prefix=cofi/" > "$LIST_JSON"
...
for dp, _, fs in os.walk(dest):
    for f in fs:
        if f.endswith('.txt'):
            local.add('cofi/' + os.path.relpath(os.path.join(dp, f), dest))
