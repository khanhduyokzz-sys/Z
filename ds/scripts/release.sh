...
notify_signal_release() {
    ...
    if [ -n "$signal_ssh_host" ]; then
        ...
        remote_cmd="SIGNAL_BOT_ENV=$quoted_env COFI_VERSION=$quoted_version COFI_TAG=$quoted_tag COFI_RELEASE_URL=$quoted_url COFI_RELEASE_NOTES=$quoted_notes SIGNAL_RELEASE_NOTIFY_DRY_RUN=$quoted_dry_run python3 -"
    ...
    COFI_VERSION="$version" \
    COFI_TAG="$tag" \
    COFI_RELEASE_URL="$release_url" \
    COFI_RELEASE_NOTES="$notes" \
...
PBXPROJ="ds.xcodeproj/project.pbxproj"
...
#
#     Rules:
#       - Untracked Cofi/tweaks/<name>.m       → "Add <Pretty Name> tweak"
...
compute_extra_bullets() {
    ...
    while IFS= read -r f; do
        ...
        add_bullet "Add ${pretty} tweak" "$pretty"
             ...
             | grep -E '^Cofi/tweaks/.*\.m$' || true)

    while IFS= read -r name; do
        [ -z "$name" ] && continue
        add_bullet "Add ${name} package" "$name"
    done < <(git diff --no-color -- Cofi/installer/PackageCatalog.m 2>/dev/null \
             | grep -E '^\+[[:space:]]+name:@"' \
             | sed -E 's/^\+[[:space:]]+name:@"([^"]+)".*/\1/' \
             | head -10)

    if printf '%s\n' "$changed_files" | grep -Eq '^Cofi/installer/'; then
        add_bullet "Polish package installer queue, badges, and activity status UI" "installer"
    fi
    if printf '%s\n' "$changed_files" | grep -Fxq 'Cofi/SettingsViewController.m'; then
        add_bullet "Track DarkSword toggle apply results independently in Settings" "settings"
    fi
    if printf '%s\n' "$changed_files" | grep -Fxq 'Cofi/LogTextView.m'; then
        add_bullet "Tighten Log tab typography for dense verbose traces" "log view"
    fi
    if printf '%s\n' "$changed_files" | grep -Fxq 'Cofi/tweaks/darksword_tweaks.m'; then
...
# 1b. Regenerate Cofi/Changelog.plist with the new version as the top entry,
...
#     CURRENT_PROJECT_VERSION. build.sh writes build/Cofi-${VERSION}.ipa and
#     refreshes a build/Cofi.ipa symlink. We build *before* committing so the
...
IPA="$PWD/build/Cofi-${VERSION}.ipa"
if [ ! -f "$IPA" ]; then
...
if [ -f "$SOURCE_JSON" ]; then
    ...
    REPO_SLUG_FOR_JSON=$(echo "$ORIGIN_URL_FOR_JSON" \
        | sed -E 's#^(https?://[^/]+/|git@[^:]+:)##' \
        | sed -E 's#\.git$##')
    DOWNLOAD_URL="https://github.com/${REPO_SLUG_FOR_JSON}/releases/download/${EFFECTIVE_TAG}/Cofi-${VERSION}.ipa"
